...
def transformation_STL_file(path, output_dir, cone_type, nb_iterations):
    ...
    # build output filename from input file
    base = os.path.basename(path)
    name, ext = os.path.splitext(base)
    file_name = f"{name}_{cone_type}_transformed{ext}"
...
file_path = r'C:\Professional\3D4E\5AxisPrinter\ConicalSlicing\3DBenchy.stl'
dir_transformed = r"C:\Professional\3D4E\5AxisPrinter\ConicalSlicing\TransformedFiles"
transformation_type = 'inward'  # inward or outward
number_iterations = 1   # number iterations for triangulation refinement

            ...
            if x_match is None and y_match is None and z_match is None:
                ...
                if inward_cone and e_match is None and (update_x or update_y):
                    ...
                    if e_match is None and np.max(z_vals) > z_max:
                        z_vals = np.minimum(z_vals, (z_max + 1))  # cut away all travel moves, that are higher than max height extruded + 1 mm safety
...
def backtransform_file(path, output_dir, cone_type, maximal_length, angle_comp, x_shift, y_shift, z_desired, e_parallel,
    ...
    file_name = f"{name}_bt_{cone_type}_{angle_comp}{ext}"
    output_path = os.path.join(output_dir, file_name)

    with open(output_path, 'w+', encoding='utf-8') as f_gcode_bt:
...
file_path = r'C:\Professional\3D4E\5AxisPrinter\ConicalSlicing\SlicedTransformedGcode\UMS3_3DBenchy_inward_transformed.gcode'
...
angle_type = 'tangential'  # radial or tangential
